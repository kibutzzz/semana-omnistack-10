...
module.exports = {
  ...
  async store(request, response) {
    try {
      ...
      if (!dev) {
        const github_response = await axios.get(
        ...
        console.log("enviar para: ",sendSocketMessageTo);
  ...
  async update(request, response) {
    try {
      ...
      let techsArray = request.body.techs
  ...
  async destroy(request, response) {
    ...
    await Dev.findByIdAndRemove(id, (err, dev) => {
      ...
      return response
        .status(204)
        .json({ message: "Dev removido com sucesso", id: dev._id });
    });
  }
};
